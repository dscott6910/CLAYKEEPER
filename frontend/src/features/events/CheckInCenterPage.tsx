...
import { ArrowLeft, CheckCircle2, CircleAlert, Loader2, Printer, RefreshCw, Search, Users } from "lucide-react"
...
export function CheckInCenterPage() {
  ...
  return <PageContainer><div className="space-y-6">
    <header className="rounded-2xl border bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm font-bold text-emerald-700">Tournament Day</p><h1 className="mt-1 text-3xl font-bold">Check-In Center</h1><p className="mt-2 text-sm text-slate-600">{data.event.name}</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => window.print()}><Printer className="h-4 w-4" />Print Report</Button><Button variant="outline" onClick={() => void load()}><RefreshCw className="h-4 w-4" />Refresh</Button></div></div>
    </header>
