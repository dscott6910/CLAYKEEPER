...
function courseToForm(course: EventCourse, stations: CourseStation[]): CourseForm {
  const courseStations = stations.filter((station) => station.course_id === course.id)
  const byNumber = new Map(courseStations.map((station) => [station.station_number, station]))
  const stationCount = Math.min(
    MAX_STATIONS_PER_COURSE,
    Math.max(1, ...courseStations.map((station) => station.station_number)),
  )
  return {
    id: course.id,
    name: course.name,
...
export function CourseBuilderPage() {
  ...
  const visibleStations = useMemo(() => form.stations.slice(0, form.stationCount), [form.stationCount, form.stations])
  const totalBirds = useMemo(() => visibleStations.reduce((total, station) => total + station.birdCount, 0), [visibleStations])
  ...
  function updateStationCount(stationCount: number) { setForm((current) => ({ ...current, stationCount })) }
  function applyToAllStations() { setForm((current) => ({ ...current, stations: current.stations.map((station) => station.stationNumber <= current.stationCount ? { ...station, birdCount: applyBirdCount } : station) })) }
  ...
  async function save() {
    ...
    try {
      const courseId = await saveEventCourse({ organizationId, eventId, courseId: form.id, name: form.name, discipline: form.discipline, courseSide: form.courseSide, templateName: form.templateName || null, stations: visibleStations })
      selectedCourseIdRef.current = courseId
  ...
  async function remove() {
    if (!form.id || !organizationId || !window.confirm(`Delete "${form.name}" and its station settings?`)) return
  ...
  return <PageContainer><div className="space-y-6">
    ...
    <section className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="rounded-2xl border bg-white shadow-sm"><div className="border-b p-4"><h2 className="font-bold text-slate-950">Event Courses</h2><p className="mt-1 text-sm text-slate-500">Saved course layouts.</p></div><div className="space-y-2 p-3">{courses.map((course) => <button key={course.id} type="button" onClick={() => chooseCourse(course.id)} className={`w-full rounded-xl border p-3 text-left transition ${selectedCourseId === course.id ? "border-slate-950 bg-slate-950 text-white" : "hover:bg-slate-50"}`}><div className="font-semibold">{course.name}</div><div className={`mt-1 text-xs ${selectedCourseId === course.id ? "text-slate-300" : "text-slate-500"}`}>{getDisciplineLabel(course.discipline)}</div></button>)}{courses.length === 0 ? <div className="rounded-xl border border-dashed p-6 text-center text-sm text-slate-500">No courses have been created.</div> : null}</div></aside>
      <main className="space-y-5">
        <section className="rounded-2xl border bg-white p-5 shadow-sm"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label><span className="text-sm font-semibold">Stations</span><select value={form.stationCount} onChange={(e) => updateStationCount(Number(e.target.value))} className="mt-1 min-h-11 w-full rounded-lg border bg-white px-3 text-sm">{Array.from({ length: MAX_STATIONS_PER_COURSE }, (_, index) => index + 1).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
