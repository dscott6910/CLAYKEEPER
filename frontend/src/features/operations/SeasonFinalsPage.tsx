      ...
      <div className="space-y-6 print:space-y-4">
        ...
        {record ? (
          ...
          <section className="rounded-2xl border border-blue-200 bg-blue-50 p-5 text-blue-950">
            <div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0" /><div><h2 className="font-bold">Season is ready for finalization</h2><p className="mt-1 text-sm text-blue-800">All available athlete-event results are complete. Finalization will freeze the individual, team, qualification, and event summaries shown here.</p></div></div>
          </section>
        ) : (
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Summary label="Athletes" value={String(record?.summary.athletes ?? draft?.standings.totals.athletes ?? 0)} detail="Season participants" />
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="rounded-2xl border bg-white p-5 shadow-sm print:shadow-none">
            ...
            <div className="mt-4 space-y-3">{teamChampions.map((row) => <div key={row.teamName} className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-xl font-black text-slate-950">{row.teamName}</p><p className="mt-1 text-sm text-slate-600">{row.athletesCounted} counting athletes · {row.eventsRepresented} events represented</p><p className="mt-2 text-sm font-bold text-emerald-800">{row.seasonPoints.toFixed(2)} team season points</p></div>)}{teamChampions.length === 0 ? <p className="text-sm text-slate-500">No team champion is available yet.</p> : null}</div>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border bg-white shadow-sm print:shadow-none">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="p-4">Rank</th><th className="p-4">Athlete</th><th className="p-4">Team / Class</th><th className="p-4 text-right">Events</th><th className="p-4 text-right">Season Points</th><th className="p-4 text-right">Average</th></tr></thead><tbody className="divide-y">{individualRows.map((row) => <tr key={row.athleteId}><td className="p-4 text-lg font-black">{row.rank}{row.tied ? <span className="ml-2 rounded-full bg-amber-100 px-2 py-0.5 text-xs text-amber-800">Tie</span> : null}</td><td className="p-4"><p className="font-bold">{row.athleteName}</p><p className="text-xs text-slate-500">{row.cyssaNumber ? `CYSSA ${row.cyssaNumber}` : "No CYSSA number"}</p></td><td className="p-4">{row.teamName}<p className="text-xs text-slate-500">Class {row.classCode}</p></td><td className="p-4 text-right">{row.eventsCounted}</td><td className="p-4 text-right font-black">{row.seasonPoints.toFixed(2)}</td><td className="p-4 text-right">{row.averagePercentage.toFixed(2)}%</td></tr>)}</tbody></table></div>
        </section>
