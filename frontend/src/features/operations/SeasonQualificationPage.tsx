...
function QualificationRow(props: { row: SeasonQualificationRow; open: boolean; toggle: () => void }) {
  ...
  return <>
    ...
    {props.open ? <tr className="bg-slate-50/70"><td colSpan={7} className="p-4"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{row.audits.map((audit) => <div key={audit.eventId} className={`rounded-xl border bg-white p-4 ${audit.state === "complete" ? "border-emerald-200" : audit.state === "incomplete" ? "border-amber-200" : "border-slate-200"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{audit.eventName}</p><p className="mt-1 text-xs text-slate-500">{formatDate(audit.startDate)}</p></div><span className={`rounded-full px-2 py-1 text-xs font-bold ${audit.state === "complete" ? "bg-emerald-100 text-emerald-800" : audit.state === "incomplete" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-700"}`}>{audit.state === "complete" ? "Counts" : audit.state === "incomplete" ? "Incomplete" : "Not Entered"}</span></div>{audit.state === "complete" ? <div className="mt-3 flex items-center gap-2 text-sm text-emerald-800"><CheckCircle2 className="h-4 w-4" />{audit.score}/{audit.targets} · {audit.percentage.toFixed(2)}%</div> : audit.state === "incomplete" ? <div className="mt-3 flex items-center gap-2 text-sm text-amber-800"><CircleAlert className="h-4 w-4" />Finish all assigned scorecards for this event to count.</div> : <p className="mt-3 text-sm text-slate-500">No season contribution recorded for this athlete.</p>}<Link to={`/events/${audit.eventId}/live-scoring`} className="mt-3 inline-flex text-xs font-bold text-emerald-700 hover:text-emerald-900">Open event scoring →</Link></div>)}</div></td></tr> : null}
  </>
}
