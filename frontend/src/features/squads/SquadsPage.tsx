...
export function SquadsPage() {
  ...
  async function moveWithinSquad(
    member: SquadMemberRecord,
    squad: SquadRecord,
    direction: -1 | 1,
  ) {
    const nextPosition = member.position + direction
    if (nextPosition < 1 || nextPosition > squad.capacity) return

    const adjacent = (membersBySquad.get(squad.id) ?? []).find(
      (row) => row.position === nextPosition,
    )
      ...
      await runAction(
        () =>
          moveMember(
            ...
            squad.id,
            nextPosition,
            positionLabel(selectedShoot?.discipline ?? "", nextPosition),
          ),
        "Participant post updated.",
      )
      return
    }
    ...
    await runAction(
      () =>
        swapMemberPositions({
          ...
          secondMemberId: adjacent.id,
          squadId: squad.id,
          firstPosition: member.position,
          secondPosition: nextPosition,
          firstLabel: positionLabel(
            selectedShoot?.discipline ?? "",
            member.position,
          ),
          secondLabel: positionLabel(
            selectedShoot?.discipline ?? "",
            nextPosition,
          ),
        }),
      "Participant posts reordered.",
    )
  }
          ...
          <>
            ...
            <div className="grid gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
              <aside className="rounded-2xl border bg-white shadow-sm">
                <div className="border-b p-4"><h4 className="font-semibold">Unassigned participants</h4><p className="mt-1 text-xs text-slate-500">All participants must be checked in before squadding.</p><div className="relative mt-3"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input className="w-full rounded-lg border py-2 pl-9 pr-3 text-sm" placeholder="Search participant, team, class, number..." value={search} onChange={(e) => setSearch(e.target.value)} /></div></div>
                      ...
                      <label key={squad.id} className="flex items-center justify-between gap-3 rounded-xl border border-amber-200 bg-white p-3 text-sm">
                        ...
                        <select
                          ...
                          onChange={(event) => {
                            ...
                            void runAction(
                              () => updateSquad(squad.id, { course_name: courseName || null }),
