...
export async function loadCheckInCenter(eventId: string): Promise<CheckInData> {
  ...
  const [athletes, teams, registrations, enrollments, squads] =
    await Promise.all([
      supabase
        .from("athletes")
      ...
      supabase
        .from("registrations")
        .select(
          "id,organization_id,event_id,athlete_id,team_id,registration_number,payment_status,amount_paid,checked_in,attendance_status,attendance_notes,refund_status,refund_amount,refund_reason,refund_notes,refund_processed_at",
        )
        .eq("organization_id", event.organization_id)
        .eq("event_id", eventId)
      ...
      supabase
        ...
        .select("id,registration_id,shoot_id")
        .eq("organization_id", event.organization_id)
        .eq("event_id", eventId)
        .eq("status", "registered"),
      supabase
        .from("squads")
        .select("id,shoot_id,squad_number")
        .eq("organization_id", event.organization_id),
    ])
  for (const result of [athletes, teams, registrations, enrollments, squads])
    check(result.error)
  ...
  const members =
    enrollmentIds.length > 0
      ? await supabase
          .from("squad_members")
          .select("registration_shoot_id,squad_id,position,position_label")
...
export async function updateRefund(input: {
  registrationId: string
  organizationId: string
...
export async function checkInRegistrations(input: {
  organizationId: string
