...
export async function loadEventOperations(requestedEventId?: string): Promise<EventOperationsSnapshot> {
  ...
  if (incompleteScores && scoreRows.length) alerts.push({ id: "scores", severity: "warning", title: "Scorecards incomplete", detail: `${incompleteScores} assigned participant(s) still have incomplete scorecards.`, path: `/events/${eventId}/digital-scoring` })
