-- ============================================================
-- ClayKeeper
-- Migration: Live scoring and shoot-offs (corrected)
--
-- Fix:
-- The scoring tables use composite foreign keys so Supabase can
-- verify that referenced records belong to the same shoot/event/
-- organization. PostgreSQL requires matching UNIQUE constraints
-- on the referenced column combinations.
-- ============================================================
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.shoots'::regclass
      and conname = 'shoots_id_event_organization_unique'
  ) then
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.squad_members'::regclass
      and conname = 'squad_members_id_shoot_organization_unique'
  ) then
    alter table public.squad_members
      add constraint squad_members_id_shoot_organization_unique
