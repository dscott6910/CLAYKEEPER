-- ClayKeeper Production QA Sprint 4
-- Make digital scorecard header + station-score writes atomic and keep scoring-role writes functional.
...
begin
  ...
  for v_station in select value from jsonb_array_elements(coalesce(p_station_scores, '[]'::jsonb)) loop
    if (v_station->>'hits')::integer < 0
       ...
       or (v_station->>'hits')::integer > (v_station->>'targets')::integer then
  ...
  if p_scorecard_id is not null then
    ...
    set status = p_status,
        ...
        finalized_at = case when p_status = 'finalized' then v_updated_at else null end,
        updated_at = v_updated_at
    where id = p_scorecard_id
      and organization_id = p_organization_id
      and event_id = p_event_id
      and shoot_id = p_shoot_id
      and squad_member_id = p_squad_member_id
    ...
    if exists (
      select 1 from public.digital_scorecards
    ...
    insert into public.digital_scorecards (
      organization_id, event_id, shoot_id, squad_member_id, course_id,
      status, malfunction_count, verified_by_1, verified_by_2,
      entered_by_name, notes, total_score, total_targets, finalized_at, updated_at
    ) values (
      p_organization_id, p_event_id, p_shoot_id, p_squad_member_id, p_course_id,
      p_status, p_malfunction_count, nullif(trim(p_verified_by_1), ''),
      nullif(trim(p_verified_by_2), ''), nullif(trim(p_entered_by_name), ''),
      nullif(trim(p_notes), ''), v_total_score, v_total_targets,
      case when p_status = 'finalized' then v_updated_at else null end, v_updated_at
    )
    returning id into v_scorecard_id;
  end if;

  delete from public.digital_scorecard_station_scores
  where scorecard_id = v_scorecard_id;

  insert into public.digital_scorecard_station_scores (
    organization_id, event_id, shoot_id, scorecard_id, station_id, hits, updated_at
  )
  select
    p_organization_id,
    p_event_id,
...
revoke all on function public.save_digital_scorecard_atomic(
  uuid, uuid, uuid, uuid, uuid, uuid, text, integer, text, text, text, text, timestamptz, jsonb
) from public, anon;
