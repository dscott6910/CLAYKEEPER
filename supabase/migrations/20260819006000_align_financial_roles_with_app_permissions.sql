-- Align financial database authorization with the application role model.
--
-- The organization_members role constraint and frontend permissions do not
-- currently define a treasurer role. Payment management is owner/admin only.
...
begin
    ...
    paid_at =
      case
        when v_payment_status = 'paid'
          then coalesce(r.paid_at, clock_timestamp())
        else null
      end,
    status =
      case
  ...
  if v_payment_status = 'paid' then
    update public.registration_shoots as rs
    set status = 'registered'
    where rs.registration_id = p_registration_id
      and rs.organization_id = p_organization_id
